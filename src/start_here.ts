...
export async function main_function_run_me() {
    // Step 1a: Make sure all required inputs are provided.
...
function set_juliaup_depot_path() {
    ...
    // RUNNER_TOOL_CACHE is documented, public GitHub Actions API, so reading it
    // directly is supported. The `@actions/tool-cache` package itself has no
    // public setter for this path, and actions/setup-java and actions/setup-python
    // both read RUNNER_TOOL_CACHE from the environment in the same way.
    // https://docs.github.com/en/actions/reference/workflows-and-actions/variables
    const tool_cache = process.env['RUNNER_TOOL_CACHE']
    if (!tool_cache || tool_cache.length === 0) {
        core.error(`RUNNER_TOOL_CACHE is not set; leaving ${env_name} unset (Juliaup will use its default location)`)
        return
    }
